...
| Bounded concurrency queue  | `priyoConcurrency.queue(...)`    | Limits how many async tasks run at the same time.   |
...
## Structured Concurrency (Groups, Queues, Cancellation, Scheduling)

`priyoConcurrency` extends async flow with grouped tasks, bounded queues, cooperative
cancellation, deadlines, combinators, and delayed task scheduling. A task group keeps related
work together so you can wait for all tasks, cancel the whole unit, and inspect task state in one
place. A queue adds the same task-handle model but caps active work with a fixed concurrency
limit.
...
### Example 4: Deadline-driven cancellation
...
### Example 5: Race, any, and allSettled
...
| API                                                           | Purpose                                        |
| ------------------------------------------------------------- | ---------------------------------------------- |
| `priyoConcurrency.group(label?)`                              | Create a task group.                           |
| `priyoConcurrency.queue(limit, label?)`                       | Create a bounded task queue.                   |
| `priyoConcurrency.after(ms, value?)`                          | Resolve a value after a delay.                 |
